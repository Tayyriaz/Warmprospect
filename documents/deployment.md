...
sudo systemctl restart chatbot || pm2 restart chatbot || {
  pkill -f "uvicorn main:app"
  nohup uvicorn main:app --host 0.0.0.0 --port 8000 > app.log 2>&1 &
}
...
tail -f app.log
# or
journalctl -u chatbot -f  # if using systemd
```
...
ssh user@72.52.132.145 "cd /var/www/chatbot && git pull && source venv/bin/activate && pip install -r requirements.txt && sudo systemctl restart chatbot"
```
